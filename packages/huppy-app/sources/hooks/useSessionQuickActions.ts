...
import { useHuppyAction } from '@/hooks/useHuppyAction';
...
import { t } from '@/text';
import { HuppyError } from '@/utils/errors';
...
export function useSessionQuickActions(
    ...
    const [resumingSession, performResume] = useHuppyAction(async () => {
        if (!resumeAvailability.canResume) {
            throw new HuppyError(resumeAvailability.message, false);
        }

        if (!machineId) {
            throw new HuppyError(t('sessionInfo.resumeSessionMissingMachine'), false);
        }
        ...
        switch (result.type) {
            ...
            case 'requestToApproveDirectoryCreation':
                throw new HuppyError(t('sessionInfo.resumeSessionUnexpectedDirectoryPrompt'), false);
            case 'error':
                throw new HuppyError(result.errorMessage, false);
        }
    });

    const [archivingSession, performArchive] = useHuppyAction(async () => {
        await maybeCleanupWorktree(session.id, session.metadata?.path, session.metadata?.machineId);

        const result = await sessionKill(session.id);
        if (!result.success) {
            throw new HuppyError(result.message || t('sessionInfo.failedToArchiveSession'), false);
        }
        onAfterArchive?.();
    });
