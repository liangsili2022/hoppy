...
function runCli(...args: string[]): { stdout: string; stderr: string; exitCode: number } {
    try {
        const stdout = execFileSync(process.execPath, [
            ...
            binPath,
            ...args,
        ], { encoding: 'utf-8', env: { ...process.env, HUPPY_HOME_DIR: '/tmp/nonexistent-happy-acceptance' } });
...
describe('Smoke: Full test suite runs', () => {
    ...
    it('config loads with correct defaults', () => {
        const origUrl = process.env.HUPPY_SERVER_URL;
        const origHome = process.env.HUPPY_HOME_DIR;
        ...
        try {
            ...
            if (origUrl !== undefined) process.env.HUPPY_SERVER_URL = origUrl;
            if (origHome !== undefined) process.env.HUPPY_HOME_DIR = origHome;
        }
    });
});
