...
describe('config', () => {
    ...
    beforeEach(() => {
        delete process.env.HUPPY_SERVER_URL;
        delete process.env.HUPPY_HOME_DIR;
    });
    ...
    describe('env var overrides', () => {
        it('overrides server URL with HUPPY_SERVER_URL', () => {
            process.env.HUPPY_SERVER_URL = 'https://custom-server.example.com';
            const config = loadConfig();
            expect(config.serverUrl).toBe('https://custom-server.example.com');
        });

        it('overrides home directory with HUPPY_HOME_DIR', () => {
            process.env.HUPPY_HOME_DIR = '/tmp/custom-happy';
            const config = loadConfig();
        ...
        it('allows both overrides simultaneously', () => {
            process.env.HUPPY_SERVER_URL = 'https://other.example.com';
            process.env.HUPPY_HOME_DIR = '/opt/happy';
            const config = loadConfig();
