...
/**
 * CLI entry point for huppy command
 * 
...
import { checkIfDaemonRunningAndCleanupStaleState, isDaemonRunningCurrentlyInstalledHuppyVersion, stopDaemon } from './daemon/controlClient'
import { getLatestDaemonLog } from './ui/logger'
import { killRunawayHuppyProcesses } from './daemon/doctor'
...
(async () => {
  ...
  if (!args.includes('--version')) {
    logger.debug('Starting huppy CLI with args: ', process.argv)
  }
  ...
  if (subcommand === 'doctor') {
    ...
    if (args[1] === 'clean') {
      const result = await killRunawayHuppyProcesses()
    ...
    // Handle "huppy gemini model set <model>" command
    ...
    // Handle "huppy gemini model get" command
    ...
    // Handle "huppy gemini project set <project-id>" command
    if (geminiSubcommand === 'project' && args[2] === 'set' && args[3]) {
      ...
      try {
        ...
        // Try to get current user email from Huppy cloud token
    ...
    // Handle "huppy gemini project get" command
    if (geminiSubcommand === 'project' && args[2] === 'get') {
      try {
        ...
        if (config.googleCloudProject) {
          ...
          console.log('  huppy gemini project set <your-project-id>');
          console.log('');
    ...
    // Handle "huppy gemini project" (no subcommand) - show help
    if (geminiSubcommand === 'project' && !args[2]) {
      console.log('Usage: huppy gemini project <command>');
      console.log('');
    ...
    try {
      ...
      logger.debug('Ensuring Huppy background service is running & matches our version...');
      if (!(await isDaemonRunningCurrentlyInstalledHuppyVersion())) {
        logger.debug('Starting Huppy background service...');
        const daemonProcess = spawnHuppyCLI(['daemon', 'start-sync'], {
          detached: true,
          stdio: 'ignore',
          env: process.env
        });
    ...
    console.log(chalk.yellow('Note: "huppy logout" is deprecated. Use "huppy auth logout" instead.\n'));
    try {
    ...
    if (daemonSubcommand === 'list') {
      ...
      const child = spawnHuppyCLI(['daemon', 'start-sync'], {
        detached: true,
        stdio: 'ignore',
        env: process.env
      });
      ...
      console.log(`
${chalk.bold('huppy daemon')} - Daemon management

${chalk.bold('Usage:')}
  huppy daemon start              Start the daemon (detached)
  huppy daemon stop               Stop the daemon (sessions stay alive)
  huppy daemon status             Show daemon status
  huppy daemon list               List active sessions

  If you want to kill all huppy related processes run
  ${chalk.cyan('huppy doctor clean')}

${chalk.bold('Note:')} The daemon runs in the background and manages Claude sessions.

${chalk.bold('To clean up runaway processes:')} Use ${chalk.cyan('huppy doctor clean')}
`)
    }
    return;
  } else {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg === '-h' || arg === '--help') {
        ...
        unknownArgs.push(arg)
      } else if (arg === '--huppy-starting-mode') {
        ...
        // Huppy-specific flag to disable chrome even if default is on
      } else if (arg === '--settings') {
        // Intercept --settings flag - Huppy uses this internally for session hooks
        const settingsValue = args[++i] // consume the value
        console.warn(chalk.yellow(`⚠️  Warning: --settings is used internally by Huppy for session tracking.`))
...
${chalk.bold('Usage:')}
  ...
  huppy gemini            Start Gemini mode (ACP)
  huppy acp               Start a generic ACP-compatible agent
...
async function handleNotifyCommand(args: string[]): Promise<void> {
  ...
  if (showHelp) {
    console.log(`
${chalk.bold('huppy notify')} - Send notification

${chalk.bold('Usage:')}
  huppy notify -p <message> [-t <title>]    Send notification with custom message and optional title
  huppy notify -h, --help                   Show this help

${chalk.bold('Options:')}
  -p <message>    Notification message (required)
  -t <title>      Notification title (optional, defaults to "Huppy")

${chalk.bold('Examples:')}
  huppy notify -p "Deployment complete!"
  huppy notify -p "System update complete" -t "Server Status"
  huppy notify -t "Alert" -p "Database connection restored"
`)
    return
  }
  ...
  if (!message) {
    ...
    console.log(chalk.gray('Run "huppy notify --help" for usage information.'))
    process.exit(1)
  }
  ...
  try {
    ...
    // Use custom title or default to "Huppy"
    const notificationTitle = title || 'Huppy'
