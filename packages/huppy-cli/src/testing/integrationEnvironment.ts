...
export function applyEnvironmentToProcess(env: IntegrationEnvironment) {
    process.env.HUPPY_SERVER_URL = `http://localhost:${env.serverPort}`;
    process.env.HUPPY_WEBAPP_URL = `http://localhost:${env.expoPort}`;
    process.env.HUPPY_HOME_DIR = join(env.envDir, 'cli', 'home');
    process.env.HAPPY_PROJECT_DIR = env.projectPath;
    process.env.HUPPY_VARIANT = 'dev';
    process.env.DEBUG = '1';
}
