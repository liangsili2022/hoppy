 ...
 */
export async function isDaemonRunningCurrentlyInstalledHuppyVersion(): Promise<boolean> {
  ...
  try {
    ...
    /*
    const { spawnHuppyCLI } = await import('@/utils/spawnHuppyCLI');
    const happyProcess = spawnHuppyCLI(['--version'], { stdio: 'pipe' });
