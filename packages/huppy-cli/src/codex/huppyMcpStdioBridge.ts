 ...
 *
 * Configure the target HTTP MCP URL via env var `HUPPY_HTTP_MCP_URL` or
...
async function main() {
  ...
  const baseUrl = urlFromArgs || process.env.HUPPY_HTTP_MCP_URL || '';

  if (!baseUrl) {
    // Write to stderr; never stdout.
    process.stderr.write(
      '[happy-mcp] Missing target URL. Set HUPPY_HTTP_MCP_URL or pass --url <http://127.0.0.1:PORT>\n'
    );
    process.exit(2);
  }
